...
- [x] `building: []`, `floor: ['obstacle', 'door', 'treasure', 'stairs']` in placement policy.
- [x] Tests updated; no test encodes **new** floor behavior as **marker** for table (table → **obstacle** + **`authoredPlaceKindId`**).
- [x] **placeObjectBridge** updated so **floor** placement does not persist **`marker`** for table; resolver + workspace carry **`authoredPlaceKindId`** where needed.
- [x] Implementer report captured in conversation + this doc’s Appendix A/C for follow-ups.
