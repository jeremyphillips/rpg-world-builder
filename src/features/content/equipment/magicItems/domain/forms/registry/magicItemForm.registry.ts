 ...
 */
import type { MagicItem, MagicItemInput } from '@/features/content/equipment/magicItems/domain/types';
...
} from '@/features/content/equipment/magicItems/domain/vocab';
...
export const MAGIC_ITEM_FORM_FIELDS = [
  ...
  >(),
  {
    ...
    kind: 'select',
    required: true,
    options: MAGIC_ITEM_SLOT_OPTIONS.map((o) => ({ value: o.id, label: o.name })),
  ...
  },
  {
    ...
    kind: 'select',
    required: true,
    options: MAGIC_ITEM_RARITY_OPTIONS.map((o) => ({ value: o.id, label: o.name })),
