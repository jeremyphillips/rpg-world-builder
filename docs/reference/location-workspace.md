...
| `LocationMapEditorPlaceTray` | Shown when `mode === 'place'`; placeables from `getPlacePaletteItemsForScale` (sets `activePlace` only); section headers from registry palette `category`. |
