...
| `GridCellViewModel` | `selectors/space.selectors.ts` | UI-ready cell with highlight flags; includes **`occupantRendersToken`**, **`occupantIsDefeated`**, **`placedObjectKind`** / **`placedObjectLabel`** (from grid objects) |
