...
- **Authoring direction:** model ally summons with **`SpawnEffect`** fields on spell effects: **`monsterId`** (repeated **`count`** times), **`monsterIds`** (explicit list), or **`pool`: `{ creatureType, maxChallengeRating }`** for random selection from `monstersById` at resolution time. Legacy **`creature`** string remains for tokens not in the catalog (e.g. familiar). Optional **`initiativeMode`**: `'group' | 'share-caster' | 'individual'` (for Phase 3). Pair with **`resolution.casterOptions`** where the rules offer a choice (form, CR tier). Encounter passes **`ResolveCombatActionOptions.monstersById`** from the ruleset catalog — see [`spawn-resolution.ts`](../../src/features/mechanics/domain/encounter/resolution/action/spawn-resolution.ts). Spell combat actions use **`targeting.kind: 'none'`** — [resolution.md — Action targeting kinds](./resolution.md#action-targeting-kinds).
